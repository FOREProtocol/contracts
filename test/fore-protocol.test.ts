...
describe("ForeProtocol", () => {
  ...
  describe("Initial state", () => {
    ...
    it("Should return proper verifiers address", async () => {
      expect(await protocol.foreVerifiers()).to.be.equal(foreVerifiers.address);
    });
  });
  ...
  describe("Change base uri", () => {
    it("Should allow to execute only by owner", async () => {
      await assertIsAvailableOnlyForOwner(async (account) => {
        return protocol.connect(account).editBaseUri("https://test.com/%.json");
      });
    });
  ...
  describe("For non created token", () => {
    it("tokenURI() should revert", async () => {
      await expect(protocol.tokenURI(1)).to.be.revertedWith("Non minted token");
    });
  });

  describe("Fore operator verification", () => {
    it("Should return false for sample account", async () => {
      expect(await protocol.isForeOperator(alice.address)).to.be.equal(false);
    });
    ...
    it("Should return true for factory", async () => {
      expect(await protocol.isForeOperator(contract.address)).to.be.equal(true);
    });
  });
  ...
  describe("Buying verifier NFT power", () => {
    beforeEach(async () => {
      ...
      await txExec(
        protocolConfig.setVerifierMintPrice(ethers.utils.parseEther("100"))
      );
    });

    it("Should revert without funds for buying power", async () => {
      await expect(
        protocol.connect(alice).buyPower(0, ethers.utils.parseEther("80"))
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    });
    ...
    describe("sucessfully", () => {
      ...
      beforeEach(async () => {
        [tx, recipt] = await txExec(
          protocol.connect(bob).buyPower(0, ethers.utils.parseEther("80"))
        );
      });
  ...
  describe("Upgrading verifier NFT", () => {
    ...
    describe("enough validations to upgrade", () => {
      ...
      it("Should revert when trying to upgrade to invalid tier", async () => {
        ...
        await expect(protocol.connect(alice).upgradeTier(1)).to.revertedWith(
    ...
    describe("not enough validations to upgrade", () => {
      ...
      it("Should revert with can't upgrade", async () => {
        await expect(protocol.connect(alice).upgradeTier(0)).to.revertedWith(
  ...
  describe("Supports interface", () => {
    ...
    it("does support ERC165", async () => {
      expect(await protocol.supportsInterface("0x01ffc9a7")).to.be.equal(true);
    });

    it("does support ERC721", async () => {
      expect(await protocol.supportsInterface("0x80ac58cd")).to.be.equal(true);
    });
  });
});
