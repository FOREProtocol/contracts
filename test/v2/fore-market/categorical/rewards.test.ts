...
describe("BasicMarketV2 / Categorical / Rewards", () => {
  ...
  describe("Valid market", () => {
    beforeEach(async () => {
      ...
      await contract
        .connect(predictorSideA1)
        ["predict(uint256,uint8)"](ethers.utils.parseEther("500"), 0);
      await contract
        .connect(predictorSideA2)
        ["predict(uint256,uint8)"](ethers.utils.parseEther("500"), 0);
      await contract
        .connect(predictorSideB1)
        ["predict(uint256,uint8)"](ethers.utils.parseEther("1000"), 1);
      await contract
        .connect(predictorSideB2)
        ["predict(uint256,uint8)"](ethers.utils.parseEther("2000"), 1);
  ...
  describe("with usdc market", () => {
    beforeEach(async () => {
      ...
      const marketHash = ethers.utils.formatBytes32String("test usdc market");
      await txExec(
        beaconFactory
          .connect(marketCreator)
          [
