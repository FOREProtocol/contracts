...
const PancakeBunniesWhitelistChecker = artifacts.require(
    "./PancakeBunniesWhitelistChecker.sol"
);
...
contract(
    ...
    ([
        ...
        // VARIABLES
        const minimumAskPrice = parseEther("0.001");
        const maximumAskPrice = parseEther("100");
        ...
        before(async () => {
            ...
            pancakeBunniesChecker = await PancakeBunniesWhitelistChecker.new(
            ...
            for (const user of [seller1, seller2, seller3]) {
                i++;
                await mockNFT1.setApprovalForAll(
                    collectibleMarket.address,
                    true,
                    { from: user }
                );
                await mockNFT2.setApprovalForAll(
                    collectibleMarket.address,
                    true,
                    { from: user }
                );
                await mockNFT3.setApprovalForAll(
                    collectibleMarket.address,
                    true,
                    { from: user }
                );

                await mockNFT1.mint("ipfs://token" + i + " .json", {
                    from: user,
                });
                await mockNFT1.mint("ipfs://token" + i + " .json", {
                    from: user,
                });
                await mockNFT2.mint("ipfs://token" + i + " .json", {
                    from: user,
                });
                await mockNFT2.mint("ipfs://token" + i + " .json", {
                    from: user,
                });
                await mockNFT3.mint("ipfs://token" + i + " .json", {
                    from: user,
                });
                await mockNFT3.mint("ipfs://token" + i + " .json", {
                    from: user,
        ...
        xdescribe("COLLECTIBLE MARKET #1 - NORMAL BEHAVIOR", async () => {
            it("Admin adds a new collection (Mock NFT #1)", async () => {
                ...
                const collections = await collectibleMarket.viewCollections(
                    "0",
                    "10"
                );
                ...
                const tokensForSale =
                    await collectibleMarket.viewAsksByCollection(
                        mockNFT1.address,
                        "0",
                        "500"
                    );
                ...
                const tokensForSaleForSeller =
                    await collectibleMarket.viewAsksByCollectionAndSeller(
                        mockNFT1.address,
                        seller1,
                        "0",
                        "500"
            ...
            it("Tokens can/cannot be listed for NFT1/NFT2", async () => {
                let result = await collectibleMarket.canTokensBeListed(
                    mockNFT1.address,
                    ["0", "1", "2", "3", "4", "5"]
                );
                ...
                result = await collectibleMarket.canTokensBeListed(
                    mockNFT2.address,
                    ["0", "1", "2", "3", "4", "5"]
                );
            ...
            it("Seller1 lists a NFT", async () => {
                const result = await collectibleMarket.createAskOrder(
                    mockNFT1.address,
                    "0",
                    parseEther("1"),
                    {
            ...
            it("Seller2 lists a NFT and modifies the order price", async () => {
                let result = await collectibleMarket.createAskOrder(
                    mockNFT1.address,
                    "2",
                    parseEther("1.2"),
                    {
                ...
                tokensForSale = await collectibleMarket.viewAsksByCollection(
                    mockNFT1.address,
                    "0",
                    "500"
                );
                ...
                ); // Price updated to 2 BNB
            });

            it("Seller2 cancels her order", async () => {
                const result = await collectibleMarket.cancelAskOrder(
                    mockNFT1.address,
                    "2",
                    {
                        from: seller2,
            ...
            it("Buyer1 matches order from seller1 with BNB", async () => {
                ...
                const estimations =
                    await collectibleMarket.calculatePriceAndFeesForCollection(
                        mockNFT1.address,
                        marketPrice
                    );
                ...
                assert.equal(
                    expectedNetPrice.toString(),
                    parseEther("0.99").toString()
                );
                assert.equal(
                    expectedTradingFee.toString(),
                ...
                const result = await collectibleMarket.buyTokenUsingBNB(
                    mockNFT1.address,
                    "0",
                    {
                        value: marketPrice,
                ...
                assert.equal(
                    String(await collectibleMarket.pendingRevenue(treasury)),
                    expectedTradingFee
                );
            ...
            it("Seller1 lists a second NFT that is bought by buyer1 with WBNB", async () => {
                ...
                result = await collectibleMarket.buyTokenUsingWBNB(
                    mockNFT1.address,
                    "1",
            ...
            it("Seller1 cannot cancel order for a token previously sold by herself", async () => {
                ...
                await collectibleMarket.createAskOrder(
                    mockNFT1.address,
                    "1",
                    parseEther("1.1"),
                    {
                ...
                await expectRevert(
                    collectibleMarket.modifyAskOrder(
                        mockNFT1.address,
                        "1",
                        parseEther("1"),
                        {
            ...
            it("Treasury claims its pending revenue", async () => {
                const result = await collectibleMarket.claimPendingRevenue({
                    from: treasury,
                });
                ...
                assert.equal(
                    String(await wrappedBNB.balanceOf(treasury)),
                    parseEther("0.021").toString()
                );

                await expectRevert(
                    collectibleMarket.claimPendingRevenue({ from: treasury }),
            ...
            it("Seller 3 adds 2 offers", async () => {
                ...
                result = await collectibleMarket.createAskOrder(
                    mockNFT1.address,
                    "5",
                    parseEther("1.5"),
                    {
            ...
            it("Admin changes minimum/max prices to 10/11 BNB, seller3 cannot change price below", async () => {
                ...
                let result =
                    await collectibleMarket.updateMinimumAndMaximumPrices(
                        newMinPrice,
                        newMaxPrice,
                        { from: admin }
                    );
                ...
                await expectRevert(
                    collectibleMarket.modifyAskOrder(
                        mockNFT1.address,
                        "5",
                        parseEther("11.0000000000000001"),
                        {
                            from: seller2,
                        }
                    ),
                ...
                await expectRevert(
                    collectibleMarket.createAskOrder(
                        mockNFT1.address,
                        "4",
                        parseEther("9.999999"),
                        {
                ...
                result = await collectibleMarket.updateMinimumAndMaximumPrices(
                    minimumAskPrice,
                    maximumAskPrice,
                    {
                        from: admin,
            ...
            it("Buyer2 buys one of the 2 collectibles listed for the second collection", async () => {
                ...
                assert.equal(
                    expectedNetPrice.toString(),
                    parseEther("1.194").toString()
                );
                assert.equal(
                    expectedTradingFee.toString(),
                    parseEther("0.0054").toString()
                );
                assert.equal(
                    expectedCreatorFee.toString(),
                    parseEther("0.0006").toString()
                );
                ...
                assert.equal(
                    String(await collectibleMarket.pendingRevenue(creator1)),
                    expectedCreatorFee
                );
            });

            it("Creator and treasury claim pending revenue", async () => {
                let result = await collectibleMarket.claimPendingRevenue({
                    from: treasury,
                });
                ...
                result = await collectibleMarket.claimPendingRevenue({
                    from: creator1,
                });
        ...
        xdescribe("COLLECTIBLE MARKET #2 - ALTERNATIVE BEHAVIOR", async () => {
            ...
            it("Cannot buy own offer", async () => {
                ...
                // Seller1 cannot purchase its own offer for 1 WBNB
                await expectRevert(
                    collectibleMarket.buyTokenUsingWBNB(
                        mockNFT2.address,
                        "0",
                        parseEther("1"),
                        {
            ...
            it("Cannot list, trade, nor modify price once collection is discontinued", async () => {
                const result =
                    await collectibleMarket.closeCollectionForTradingAndListing(
                        mockNFT2.address,
                        { from: admin }
                    );

                expectEvent(result, "CollectionClose", {
                    collection: mockNFT2.address,
                });
        ...
        xdescribe("COLLECTIBLE MARKET #3 - TOKEN RESTRICTIONS/PANCAKEBUNNIES", async () => {
            it("Add collection with restrictions", async () => {
                ...
                const tokenListingStatuses =
                    await collectibleMarket.canTokensBeListed(
                        pancakeBunnies.address,
                        ["0", "1", "2", "3", "4", "5"]
                    );
            ...
            it("Owner mint bunnyId 1-5 for seller1 and owner adds restrictions for bunnyId 3/4", async () => {
                ...
                const result =
                    await pancakeBunniesChecker.addRestrictionForBunnies([
                        new BN("3"),
                        new BN("4"),
                    ]);
                ...
                assert.equal(
                    await pancakeBunniesChecker.isBunnyIdRestricted("3"),
                    true
                );
                assert.equal(
                    await pancakeBunniesChecker.isBunnyIdRestricted("4"),
                    true
                );
            ...
            it("Seller 1 can sell tokenIds 0-2 (bunnyIds 0-2)", async () => {
                await pancakeBunnies.setApprovalForAll(
                    collectibleMarket.address,
                    true,
                    { from: seller1 }
                );
            ...
            it("Owner removes restrictions for bunnyId=4", async () => {
                let result =
                    await pancakeBunniesChecker.removeRestrictionForBunnies([
                        new BN("4"),
                    ]);
            ...
            it("Revert statements work as expected", async () => {
                await expectRevert(
                    pancakeBunniesChecker.removeRestrictionForBunnies(
                        [new BN("3"), new BN("4")],
                        { from: owner }
                    ),
                ...
                await expectRevert(
                    pancakeBunniesChecker.addRestrictionForBunnies(
                        [new BN("3"), new BN("4")],
                        { from: owner }
                    ),
        ...
        xdescribe("COLLECTIBLE MARKET #4 - ADMIN/OWNER/SPECIAL BEHAVIOR", async () => {
            it("Can recover tokens sent by accident", async () => {
                ...
                let result = await collectibleMarket.recoverFungibleTokens(
                    mockERC20.address,
                    { from: owner }
                );
                ...
                await mockNFT1.transferFrom(
                    buyer1,
                    collectibleMarket.address,
                    "1",
                    { from: buyer1 }
                );
                result = await collectibleMarket.recoverNonFungibleToken(
                    mockNFT1.address,
                    "1",
                    { from: owner }
                );
                expectEvent(result, "NonFungibleTokenRecovery", {
                    token: mockNFT1.address,
                    tokenId: "1",
                });

                // MockNFT3 (collection not approved)
                await mockNFT3.transferFrom(
                    seller2,
                    collectibleMarket.address,
                    "3",
                    { from: seller2 }
            ...
            it("Cannot recover if nothing to, WBNB or NFTs that are listed", async () => {
                ...
                await expectRevert(
                    collectibleMarket.recoverFungibleTokens(mockERC20.address, {
                        from: owner,
                    }),
                ...
                // Cannot recover WBNB
                await wrappedBNB.transfer(
                    collectibleMarket.address,
                    parseEther("1"),
                    { from: buyer3 }
                );
                await expectRevert(
                    collectibleMarket.recoverFungibleTokens(
                        wrappedBNB.address,
                        { from: owner }
                    ),
                ...
                await expectRevert(
                    collectibleMarket.recoverNonFungibleToken(
                        mockNFT2.address,
                        "0",
                        { from: owner }
                    ),
            ...
            it("Cannot modify or remove a collection if not listed", async () => {
                ...
                await expectRevert(
                    collectibleMarket.closeCollectionForTradingAndListing(
                        mockNFT3.address,
                        { from: admin }
                    ),
                    "Operations: Collection not listed"
            ...
            it("Cannot change min/max prices if maxPrice >= minPrice", async () => {
                await expectRevert(
                    collectibleMarket.updateMinimumAndMaximumPrices(
                        minimumAskPrice,
                        minimumAskPrice,
                        { from: admin }
                    ),
            ...
            it("Only admin can call admin functions", async () => {
                for (const user of [buyer1, seller1, owner]) {
                    await expectRevert(
                        collectibleMarket.addCollection(
                            mockNFT2.address,
                            creator1,
            ...
            it("Only owner can call owner functions", async () => {
                for (const user of [buyer1, seller1, admin]) {
                    await expectRevert(
                        collectibleMarket.recoverFungibleTokens(
                            mockERC20.address,
                            { from: user }
                    ...
                    await expectRevert(
                        collectibleMarket.setAdminAndTreasuryAddresses(
                            seller1,
                            seller2,
                            { from: user }
                        ),
            ...
            it("Owner can change admin/treasury but cannot if one of them is equal to 0x address", async () => {
                const result =
                    await collectibleMarket.setAdminAndTreasuryAddresses(
                        admin,
                        treasury,
                        {
                            from: owner,
                ...
                expectEvent(result, "NewAdminAndTreasuryAddresses", {
                    admin: admin,
                    treasury: treasury,
                });
            ...
            xdescribe("COLLECTIBLE MARKET #5 - VIEW FUNCTIONS", async () => {
                it("Add fourth collection whose tokens are minted/listed by seller3", async () => {
                    ...
                    while (i < 30) {
                        await mockNFT4.mint("ipfs://token" + i + " .json", {
                            from: seller3,
                        });
                        ...
                        await mockNFT4.setApprovalForAll(
                            collectibleMarket.address,
                            true,
                            { from: seller3 }
                        );
                ...
                it("viewCollections", async () => {
                    let collections = await collectibleMarket.viewCollections(
                        "0",
                        "1"
                    );
                    assert.equal(String(collections[2]), "1"); // 3 collections
                    collections = await collectibleMarket.viewCollections(
                        "1",
                        "1"
                    );
                ...
                it("viewAsksByCollectionAndTokenIds", async () => {
                    const collections =
                        await collectibleMarket.viewAsksByCollectionAndTokenIds(
                            mockNFT4.address,
                            [
                                new BN("0"),
                                new BN("1"),
                    ...
                    assert.equal(
                        String(collections[1][2][1]),
                        parseEther("1.2").toString()
                    );
                    assert.equal(
                        String(collections[1][3][1]),
                        parseEther("0").toString()
                    );
                });
                ...
                it("viewAsksByCollection", async () => {
                    ...
                    collections = await collectibleMarket.viewAsksByCollection(
                        mockNFT4.address,
                        "10",
                        "15"
                    );
