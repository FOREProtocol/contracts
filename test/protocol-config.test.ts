...
describe("Protocol configuration", () => {
  ...
  describe("Initial state", () => {
    ...
    it("Should expose proper high guard account", async () => {
      expect(await contract.highGuard()).to.be.equal(highGuardAccount.address);
    });
    ...
    it("Should expose proper FORE token address", async () => {
      expect(await contract.foreToken()).to.be.equal(foreTokenContract.address);
    });
    ...
    it("Should return proper whitelist status", async () => {
      expect(await contract.isFactoryWhitelisted(owner.address)).to.be.equal(
        false
      );
    });
    ...
    it("Should return proper addresses", async () => {
      ...
      expect(marketConfigAddress).to.be.equal(await contract.marketConfig());
      expect(foundationWalletAddress).to.be.equal(foundationWallet.address);
      ...
      expect(foreVerifiersAddress).to.be.equal(foreVerifiersContract.address);
    });
    ...
    describe("Market configuration", () => {
      ...
      it("Should expose proper verification period", async () => {
        expect(await marketConfig.verificationPeriod()).to.be.equal(86400);
      });
  ...
  describe("Change tier config", () => {
    ...
    it("Should not allow 0 multiplier", async () => {
      await expect(contract.connect(owner).editTier(0, 0, 0)).to.revertedWith(
  ...
  describe("Set whitelist status", () => {
    describe("Successful", () => {
      ...
      beforeEach(async () => {
        [tx, recipt] = await txExec(
          contract.connect(owner).setFactoryStatus([alice.address], [true])
        );
      });
      ...
      it("Should emit SetStatusForFactory event", async () => {
        assertEvent<SetStatusForFactoryEvent>(recipt, "SetStatusForFactory", {
          add: alice.address,
          status: true,
        });
      });
      ...
      it("Should have enabled status", async () => {
        expect(await contract.isFactoryWhitelisted(alice.address)).to.be.equal(
          true
        );
      });
      ...
      describe("Edit few statuses", () => {
        beforeEach(async () => {
          [tx, recipt] = await txExec(
            contract
              .connect(owner)
              .setFactoryStatus([alice.address, owner.address], [false, true])
          );
        });
    ...
    describe("Invalid status", () => {
      ...
      it("Should revert when empty factory address", async () => {
        await expect(
          contract
            .connect(owner)
            .setFactoryStatus([ethers.constants.AddressZero], [true])
        ).to.be.revertedWith("ProtocolConfig: Factory address cannot be zero");
      });
    });
  });
  ...
  describe("Change foundation wallet", () => {
    it("Should allow to execute only by owner", async () => {
      await assertIsAvailableOnlyForOwner(async (account) => {
        return contract.connect(account).setFoundationWallet(alice.address);
      });
    });
    ...
    it("Should revert when ZERO address", async () => {
      ...
      ).to.be.revertedWith("ProtocolConfig: Foundation address cannot be zero");
    });
  ...
  describe("Change high guard account", () => {
    ...
    it("Should revert when ZERO address", async () => {
      await expect(
        contract.connect(owner).setHighGuard(ethers.constants.AddressZero)
      ).to.be.revertedWith("ProtocolConfig: HighGuard address cannot be zero");
    });
  ...
  describe("Change marketplace contract address", () => {
    ...
    it("Should revert when ZERO address", async () => {
      await expect(
        contract.connect(owner).setMarketplace(ethers.constants.AddressZero)
      ).to.be.revertedWith(
  ...
  describe("Change market creation price", () => {
    ...
    it("Should emit MarketCreationChanged event", async () => {
      ...
      assertEvent<MarketCreationChangedEvent>(recipt, "MarketCreationChanged", {
        amount: ethers.utils.parseEther("5"),
      });
    });
  });
...
describe("Constructor arguments", () => {
  ...
  it("Should revert with empty fore verifiers address", async () => {
    ...
    ).to.revertedWith("ProtocolConfig: FOREVerifiers address cannot be zero");
  });
});
