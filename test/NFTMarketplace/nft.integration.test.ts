...
import { NFTMarketplace, TradeEvent } from "@/NFTMarketplace";
...
describe("NFTMarketplace / NFT integration", () => {
    ...
    let nftContract: ForeAnalystNFT;
    ...
    async function createTokens(num: number, recipients: string[]) {
        for (let i = 0; i < num; ++i) {
            ...
            const [tx, recipt] = await txExec(
                nftContract.connect(owner).mint(recipient, {
                    staked: 1,
                    penalty: 1,
                })
            );
    ...
    beforeEach(async () => {
        ...
        nftContract = await deployContractAs<ForeAnalystNFT>(
            creator,
            "ForeAnalystNFT"
        );
