...
export async function deployUniversalRouter(
  ...
  const permit2 = await permitFactory.deploy();

  const RouterFactory = await ethers.getContractFactory("ForeUniversalRouter");
  const contract = await upgrades.deployProxy(
    RouterFactory,
    [accessManagerAddress, protocolAddress, permit2.address, tokenAddresses],
    {
      kind: "uups",
